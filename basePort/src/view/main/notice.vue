...
import { getStatusNoticeBase } from '@/api/notifyApi'
import useStore from '@/store';
import { storeToRefs } from 'pinia';
let { commonStore } = useStore()
let { notifyTypeList } = storeToRefs(commonStore);
...
const onGetPagingData = async () => {
    let { data } = await getStatusNoticeBase();
...
const formatType = (type: number): string => {
    if (notifyTypeList.value.length === 0) return '';
    return notifyTypeList.value.filter(item => {
...
onMounted(async () => {
    ...
    if (notifyTypeList.value.length === 0) await commonStore.getNotifyTypeFn()
})
