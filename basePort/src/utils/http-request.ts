...
import { getToken, getRefToken, setToken, setRefToken } from "./auth";
import { refreshToken } from "@/api/basePortApi/index";
...
// 免登陆请求白名单
let WhiteList: Array<string> = ["/sysUserBase/refreshToken"];

// 请求错误提示白名单
let errorWhiteList: Array<string> = [
  "/sysUser/checkRepeat",
  "/sysUser/checkRepeatByPhone",
  "/sysUser/checkRepeatByEmail",
  "/sysUserBase/checkRepeat",
  "/sysUserBase/checkRepeatByPhone",
  "/sysUserBase/checkRepeatByEmail",
  "/inTheEntity/checkRepeatByProtocolNum",
  "/sysUserBase/token",
];
  ...
  (config) => {
    if (getToken() && getRefToken()) {
      (config.headers as AxiosRequestHeaders)["Authorization"] =
        config.url !== "/sysUserBase/refreshToken" ? getToken() : getRefToken();
    }
    let cancel;
    // 设置cancelToken对象
