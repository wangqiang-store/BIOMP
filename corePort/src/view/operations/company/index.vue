            ...
            <el-dialog class="custom-class" v-model="closedDialogVisible" title="批量关闭" width="30%" destroy-on-close>
                <div>
                    ...
                    <el-select class="w-100 mb-20" v-model="selectCloseEntity" multiple filterable
                        ...
                        <el-option v-for="item in runEntityList" :key="item.id" :label="item.name" :value="item" />
                    </el-select>
...
let { runEntityList, entityStatus, entityType, allBaseList } = storeToRefs(commonStore)
...
const onBatchClosed = () => {
    ...
    commonStore.onGetRunEntityData();
};
// 切换表格显示个数
...
onMounted(async () => {
    ...
    baseOptions.value = allBaseList.value as Array<any>;
    // 获取实体类型
