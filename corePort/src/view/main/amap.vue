...
let { runBaseList } = storeToRefs(commonStore);
...
onMounted(async () => {
    await commonStore.onGetRunBaseData();
    (runBaseList.value as Array<{ id: number; name: string; address?: string; longitude: number; latitude: number; label: string }>).forEach(item => {
        baseList.value.push({
            name: item.name,
