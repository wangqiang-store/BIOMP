                ...
                <template #title>
                    <div class="w-100 d_flex d_flex_sb d_flex_ac">
                        ...
                        <span>
                            ...
                            {{ item.workingTime ? dayjs(item.workingTime[1]).format("YYYY.MM") : '' }}
                        </span>
                        <el-button class="mr-5" text type="primary" @click="onCancel(item, index)">删除</el-button>
                    </div>
                </template>
                <reuse-form :form-ref="`formRef${index} `" :form-data="workListFormData[index]"
                    ...
                    <template #workingTime>
                        ...
                        <el-date-picker v-model="workListFormData[index].endTime" type="month"
                            @change="(val: any) => onTimeChange('endTime', val, workListFormData[index])"
                            placeholder="结束时间" />

                        <!-- <el-date-picker style="width: 50%;" v-model="workListFormData[index].workingTime"
                            type="monthrange" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间" /> -->
                    </template>
                </reuse-form>
            </el-collapse-item>
        </el-collapse>
...
// 工作经历 折叠栏展示数据
let workList = ref<Array<workDataType>>([]);
// 折叠栏内容展示数据
