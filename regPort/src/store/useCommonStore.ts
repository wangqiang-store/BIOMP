...
const useCommonStore = defineStore("common", {
  state(): useCommonStoreState {
    return {
      ...
      baseOwnership: [], //基地权属
      baseType: [], //基地权属
      runCodeBaseList: null, //当前区域正在运行基地列表
      allCodeBaseList: null, //当前区域所有基地列表
      allProvinceList: [], //所有省列表
      entityType: [], //实体类型
      entityStatus: [], //实体状态
      runCodeEntityList: null, //孵化中实体
      activityType: [], //活动类型
      mentorType: [], //导师类型
      genderType: [],
      notifyType: [], //通告类型
  ...
  actions: {
    ...
    // 获取当前区域还在运营中的基地
    async onGetRunBaseData(code: number) {
      let { data } = await getRunBase({ countyCode: code });
      if (data.code !== 200) return;
      // 根据创建时间排序
      data?.data.sort(
      ...
      this.runCodeBaseList = data?.data;
    },
    // 获取当前区域所有基地
    async onGetAllBaseData(code: number) {
      let { data } = await getAllBase({ countyCode: code });
      if (data.code !== 200) return;
      // 根据创建时间排序
      data?.data.sort(
        (a: { createTime: string }, b: { createTime: string }) =>
      ...
      this.allCodeBaseList = data?.data;
    },
    // 获取当前区域还在孵化中的实体
    async onGetRunEntityData(code: number) {
      let { data } = await getRunEntity({ countyCode: code });
      if (data.code !== 200) return;
      this.runCodeEntityList = data?.data;
    },
    // 获取基地权属
