    ...
    <el-row class="p-20">
        <el-col :span="24" class="d_flex d_flex_ac mt-20">
            <div class="d_flex d_flex_ac mr-10">
                <!-- <el-input v-model="baseName" placeholder="孵化基地名称"></el-input> -->
            ...
            <el-dialog class="custom-class" v-model="closedDialogVisible" title="批量退出" width="30%" destroy-on-close>
                <div>
                    ...
                    <el-select class="w-100 mb-20" v-model="selectCloseBase" multiple filterable placeholder="输入基地名称关键词"
                        ...
                        <el-option v-for="item in runCodeBaseList" :key="item.id" :label="item.name" :value="item" />
                    </el-select>
...
let { baseLevel, runCodeBaseList, allCodeBaseList } = storeToRefs(commonStore);
...
let retainTableData = ref<Array<HatchTableRow>>([]);
...
const querySearch = (queryString: string, cb: any) => {
    if (allCodeBaseList.value === null) return;
    let allBaseData = allCodeBaseList.value.map(item => {
...
const onBatchClosed = () => {
    ...
    commonStore.onGetRunBaseData(userInfo.value.countyCode);
};
// 切换表格显示个数
