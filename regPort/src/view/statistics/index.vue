<template>
    <el-row style="padding: 30px;padding-bottom: 50px;">
        ...
        <el-col :span="24" style="height: 38px;" class="mt-20 mb-20 d_flex d_flex_end">
        ...
        <el-col :span="8" :xs="24" :sm="24" :md="24" :lg="12" :xl="8">
            <StatisticsChart :chart-data="statisticsData" page-name="statistics" title="入孵创业实体相关统计"
                :legend-option="statLegendOption" />
        </el-col>
        <el-col :span="24" class="mt-20">
            <h3>{{ yearText }}年第{{ quarterText }}季度{{ provinceName }}({{ countyName }})创业孵化基地汇总情况表</h3>
...
import { getBaseStat, getRunBase } from '@/api/core/hatchBase';
import { getEntityStat, getBaseLevelNum, getTotalAreaDto } from '@/api/core/statistics';
import { getByProvinceCode, getByCountyCode } from '@/api/enum';
...
let regionCode = ref<number | null>(userInfo.value.countyCode);
// 省名称
        ...
        switch (level) {
            case 0:
                ...
                allCountylist.value.forEach(item => {
                    if (userInfo.value.countyCode === item.status) {
                        countyData.push({
                            value: item,
            ...
            case 1:
                // 根据区获取基地
                let { data } = await getRunBase({ countyCode: value.status })
...
const initStatData = (quarter: number | string, year: number | string) => {
    ...
    onGetEntityStatData(regionCode.value, formatTime);
    onGetBaseLevelNum(regionCode.value, formatTime);
    onGetBaseStatOrArea(regionCode.value, formatTime)
}
...
const onExportExcel = async () => {
    ...
    let { data } = await exportStatisticsExcel({ countyCode: regionCode.value, time: formatTime });
    ...
    a.href = blobURL;
    a.download = `${yearText.value}年第${quarterText.value}季度${provinceName.value}(${countyName.value})创业孵化基地汇总情况表`;
    a.click()
}

// 获取当前省
const onGetCurrentProvince = async () => {
    let { data } = await getByProvinceCode({ code: userInfo.value.provinceCode });
    if (data.code !== 200) return;
    provinceName.value = data.data.name
}

// 获取当前区
const onGetCurrentCount = async () => {
    let { data } = await getByCountyCode({ code: userInfo.value.countyCode });
    if (data.code !== 200) return;
    countyName.value = data.data.name
}

onMounted(() => {
    onGetCurrentProvince();
    onGetCurrentCount();
});
</script>
